...
do
	...
	do
	for testset in "test_set_diff.src.constraint_suffix.sp" "test_set_diff.src.sp" "test_set_diff.src.constraint_correct_sf_suffix.sp" "test_set_diff.src.constraint_lemmatized_suffix.sp" "test_set_diff.src.constraint_dict+correct_sf_suffix.sp"	"test_set_diff.src.constraint_dict+lemmatized_suffix.sp" "test_set.src.constraint_dict.sp" "test_set.src.constraint_dict_lemmatized.sp"
	do
		cat europarl/$testset | ../marian-constraints_prom/build/marian-decoder -v $src_vocab $tgt_vocab -m models/$model  $shift -d 2  --mini-batch 16 --max-length-crop --max-length 100 --max-length-factor 2 --maxi-batch-sort=src --maxi-batch=50 | $spm_dir/spm_decode --model ../corp/encs.model  > out/"$model""$shift"_"$testset".out
		cat out/"$model""$shift"_"$testset".out | bash lemm_preserve_newlines.sh > out/"$model""$shift"_"$testset".out.lemmatized
		python3 const_coverage_and_placement.py europarl/test_set_diff.constraints.lemmatized  out/"$model""$shift"_"$testset".out.lemmatized europarl/test_set_diff.tgt > out/"$model""$shift"_"$testset".lemma_const_coverage
		python3 const_coverage_and_placement.py europarl/test_set_diff.constraints_correct_sf  out/"$model""$shift"_"$testset".out europarl/test_set_diff.tgt > out/"$model""$shift"_"$testset".surface_const_coverage
		cat out/"$model""$shift"_"$testset".out | python3 -m sacrebleu europarl/test_set_diff.tgt >  out/"$model""$shift"_"$testset".surface_bleu
		cat out/"$model""$shift"_"$testset".out.lemmatized | python3 -m sacrebleu europarl/test_set_diff.tgt >   out/"$model""$shift"_"$testset".lemma_bleu
