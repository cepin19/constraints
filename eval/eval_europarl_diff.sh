...
do
	...
	do
	for testset in "test_set_diff.src.constraint_suffix.sp" "test_set_diff.src.sp" "test_set_diff.src.constraint_correct_sf_suffix.sp"	
	do
		...
		python3 const_coverage_and_placement.py europarl/test_set_diff.constraints_correct_sf  out/"$model"_"$testset".out.lemmatized europarl/test_set.tgt.diff > out/"$model"_"$testset".lemma_const_coverage
		python3 const_coverage_and_placement.py europarl/test_set_diff.constraints_correct_sf  out/"$model"_"$testset".out europarl/test_set.tgt.diff > out/"$model"_"$testset".surface_const_coverage
		cat out/"$model"_"$testset".out | python3 -m sacrebleu europarl/test_set_diff.tgt >  out/"$model"_"$testset".surface_bleu
		cat out/"$model"_"$testset".out.lemmatized | python3 -m sacrebleu europarl/test_set_diff.tgt >   out/"$model"_"$testset".lemma_bleu
